...
function ordersDependencies(overrides: Partial<AppDependencies> = {}) {
  ...
  const values: AppDependencies & { close: ReturnType<typeof vi.fn> } = {
    ...
    fetchInvoicingSnapshot: vi.fn(async () => ({
      amountToInvoice: 0,
      notFullyInvoiced: [],
      warrantyMissing: [],
    })),
...
describe('Orders API', () => {
  it('serves the paged contract and resets filters to the empty universe', async () => {
    const deps = ordersDependencies({
      fetchPagedOrderSummaries: vi.fn(async () => ({
        items: [summaryRow],
        nextCursor: null,
        total: 1,
      })),
    })
    const response = await ordersApi(deps).post('/api/orders/page').send({ limit: 300 })

    expect(response.status).toBe(200)
    expect(response.body).toEqual({ ok: true, items: [summaryRow], nextCursor: null, total: 1 })
    expect(deps.fetchPagedOrderSummaries).toHaveBeenCalledWith(expect.anything(), {
      filters: {},
  ...
  it('rejects a cursor with a non-default sort before opening a connection', async () => {
    const deps = ordersDependencies({ fetchPagedOrderSummaries: vi.fn() })
  ...
  it('lists orders through the managed profile without a default limit', async () => {
    const deps = ordersDependencies()
    ...
    expect(deps.fetchOrderSummaries).toHaveBeenCalledWith(
      expect.anything(),
      {},
      undefined,
      undefined,
  ...
  it('forwards filters and a custom limit to the repository', async () => {
    ...
    expect(response.status).toBe(200)
    expect(deps.fetchOrderSummaries).toHaveBeenCalledWith(expect.anything(), filters, 50, undefined)
  })

  it('rejects a limit above 10000 before opening a connection', async () => {
    const deps = ordersDependencies()
    const response = await ordersApi(deps).post('/api/orders/list').send({ limit: 10001 })
  ...
  it('forwards filters to fetchOrderFacets and returns the resolved facet set', async () => {
    const deps = ordersDependencies({
      fetchOrderFacets: vi.fn(async () => ({
    ...
    const filters = { idArea: ['BDAL'], idTipo: ['INSTR'] }
    const response = await ordersApi(deps).post('/api/orders/facets').send({ filters })
    ...
    expect(deps.fetchOrderFacets).toHaveBeenCalledWith(expect.anything(), filters)
  })
})
...
describe('Orders update endpoint', () => {
  ...
  it('accepts order-level Email and Contacto updates', async () => {
    const deps = ordersDependencies()
    const patch = { Email: 'updated@example.test', Contacto: 'Updated Contact' }
    const response = await ordersApi(deps, null)
      .post('/api/orders/update')
  ...
  it('allows a user to fill blank warranty fields on a histórico order', async () => {
    const blankWarrantyOrder = {
      ...historicoDetailRow,
      ID_Tp_Warranty: null,
      Warranty_DT_Inicio: null,
    }
    const deps = ordersDependencies({
      fetchOrderById: vi.fn(async () => blankWarrantyOrder),
    })
    const response = await ordersApi(deps, null)
      .post('/api/orders/update')
      .set('x-user-role', 'user')
      .send({
        id: 202,
        patch: { ID_Tp_Warranty: 2, Warranty_DT_Inicio: '2026-08-25' },
      })

    expect(response.status).toBe(200)
    expect(deps.updateOrder).toHaveBeenCalledWith(expect.anything(), 202, {
      ID_Tp_Warranty: 2,
      Warranty_DT_Inicio: '2026-08-25',
      user: 'editor',
    })
  })

  it('allows a user to update existing warranty fields on a histórico order', async () => {
    ...
    const response = await ordersApi(deps, null)
      ...
      .send({
        id: 202,
        patch: { ID_Tp_Warranty: 3, Warranty_DT_Inicio: '2026-08-25' },
      })
...
describe('Orders create endpoint', () => {
  ...
  it('allows a non-Client order with only order type and Client', async () => {
    const deps = ordersDependencies()
    const body = { ID_Tp_Order: 'WPO', ID_Client: 93, Encomenda_Cli_PHC: 'SAP-9101' }
    const response = await ordersApi(deps, null)
      .post('/api/orders')
      .set('x-user-role', 'user')
      .send(body)
    ...
    expect(response.status).toBe(201)
    expect(deps.createOrder).toHaveBeenCalledWith(expect.anything(), body, 'editor')
  })
  ...
  it.each(['ID_Instrumento', 'Sell_Price'] as const)(
    'rejects a create without mandatory %s',
    async (field) => {
      const deps = ordersDependencies()
      const body: Partial<typeof validCreateBody> = { ...validCreateBody }
      delete body[field]
      const response = await ordersApi(deps, null)
        .post('/api/orders')
        .set('x-user-role', 'user')
...
describe('Orders warranty years endpoint', () => {
  it("updates N_Anos for the order's matched warranty type", async () => {
    const deps = ordersDependencies({
      updateOrderWarrantyYears: vi.fn(async () => true),
    })
    const response = await ordersApi(deps, null)
      .post('/api/orders/warranty-years')
      .set('x-user-role', 'user')
      .send({ id: 101, years: 4 })

    expect(response.status).toBe(200)
    expect(deps.updateOrderWarrantyYears).toHaveBeenCalledWith(expect.anything(), 101, 4, 'editor')
  })

  it('returns 400 when the order has no warranty type set', async () => {
    const deps = ordersDependencies({
      updateOrderWarrantyYears: vi.fn(async () => false),
    })
    const response = await ordersApi(deps, null)
      .post('/api/orders/warranty-years')
      .set('x-user-role', 'user')
  ...
  it('rejects viewers with 403 forbidden', async () => {
    ...
    expect(response.status).toBe(403)
    expect(deps.updateOrderWarrantyYears).not.toHaveBeenCalled()
  })

  it('rejects invalid years and id via zod', async () => {
    const deps = ordersDependencies()
    const yearsZero = await ordersApi(deps, null)
      .post('/api/orders/warranty-years')
      .send({ id: 101, years: 0 })
    const yearsFraction = await ordersApi(deps, null)
      .post('/api/orders/warranty-years')
      .send({ id: 101, years: 2.5 })
    const yearsTooBig = await ordersApi(deps, null)
      .post('/api/orders/warranty-years')
      .send({ id: 101, years: 200 })
    const badId = await ordersApi(deps, null)
      .post('/api/orders/warranty-years')
      .send({ id: 0, years: 3 })

    expect(yearsZero.status).toBe(400)
    expect(yearsFraction.status).toBe(400)
    expect(yearsTooBig.status).toBe(400)
