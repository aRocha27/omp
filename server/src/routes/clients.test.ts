...
describe('Clients API', () => {
  it('lists clients through the managed profile with the default limit', async () => {
    ...
    expect(deps.fetchClientSummaries).toHaveBeenCalledWith(expect.anything(), {}, 200)
  ...
  it('rejects a limit above 1000 before opening a connection', async () => {
    const deps = clientsDependencies()
    const response = await clientsApi(deps).post('/api/clients/list').send({ limit: 1001 })
