...
import { TableUnavailableError } from '../repositories/database.js'
import { StockInsufficientError } from '../repositories/stock.repository.js'
import {
  ClientNotFoundError,
...
const poolCacheByFactory = new WeakMap<
  (connection: ConnectionConfig) => Promise<ConnectionPool>,
  Map<string, Promise<ConnectionPool>>
>()
...
export async function withPool<T>(
  ...
  let poolCache = poolCacheByFactory.get(openPool)
  if (!poolCache) {
    poolCache = new Map<string, Promise<ConnectionPool>>()
    poolCacheByFactory.set(openPool, poolCache)
  }
  const key = JSON.stringify(connection)
  let poolPromise = poolCache.get(key)
  if (!poolPromise) {
    poolPromise = openPool(connection).catch((error) => {
