...
export function toConnectError(err: unknown): ConnectError {
  ...
  // SQL request/query failures, including constraint and conversion errors. Keep the
  // driver message out of the response because it may contain SQL details or values.
  if (
    code === 'EREQUEST' ||
    code === 'EQUERY' ||
    ...
    return new ConnectError(
      'unknown',
      'Database operation failed. Check the required fields and the table constraints.',
    )
  }
