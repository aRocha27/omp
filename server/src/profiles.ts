...
export function listProfileMetadata(environment: Environment = process.env): ProfileMetadata[] {
  return profileIds(environment).map((id) => {
    ...
    return {
      ...
      active: (environment.ORDERS_PROFILE_ID?.trim() || profileIds(environment)[0]) === id,
      networkMode: readNetworkMode(
        environment[`${prefix}NETWORK_MODE`] ?? environment[`${prefix}NETWORK`],
        id,
      ),
      server: required(environment, `${prefix}SERVER`, id),
      port: readPort(environment[`${prefix}PORT`], id),
      database: environment[`${prefix}DATABASE`]?.trim() || undefined,
      user: required(environment, `${prefix}USER`, id),
    }
  })
}
