...
export class MockDataStore {
  ...
  /**
   * Mock stand-in for `dbo.Tp_Warranty.N_Anos`. Live backend persists the
   * years on the matched warranty type row, so per-type buckets are the
   * simplest mirror — the snapshot rebuilds the per-order value from this map.
   */
  readonly warrantyYearsByType: Record<number, number> = {}
