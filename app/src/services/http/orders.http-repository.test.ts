...
describe('HttpOrdersRepository', () => {
  ...
  describe('search', () => {
    it('POSTs to /orders/list with normalised filters and no default limit, returns summaries', async () => {
    ...
    it('forwards limit and offset when a page is requested', async () => {
      fetchMock.mockResolvedValueOnce(jsonResponse(200, { ok: true, orders: [summaryRow] }))

      await repo.search({}, { limit: 500, offset: 1000 })

      const init = fetchMock.mock.calls[0][1] as RequestInit
      expect(JSON.parse(init.body as string)).toEqual({ filters: {}, limit: 500, offset: 1000 })
    })
  ...
  describe('getById', () => {
    ...
    it('strips upsize_ts while preserving order contact fields from the live row', async () => {
      ...
      fetchMock.mockResolvedValueOnce(
        jsonResponse(200, {
          ...
          order: {
            ...detailRow,
            upsize_ts: 'should-not-survive',
            Email: 'live@example.test',
            Contacto: 'Live Contact',
          },
        }),
      )
      ...
      expect(order?.Email).toBe('live@example.test')
      expect(order?.Contacto).toBe('Live Contact')
    })
