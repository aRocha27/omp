...
describe('OrdersPage', () => {
  ...
  it('narrows results when filtering by client name', async () => {
    ...
    await waitFor(() => expect(screen.queryByText('Client Beta')).not.toBeInTheDocument())
  })
  ...
  it('narrows results when filtering by SAP ref', async () => {
    ...
    await waitFor(() => expect(screen.queryByText('PHC-1002')).not.toBeInTheDocument())
  })

  it('renders every known live area/product code as a readable label, not "—"', async () => {
    renderWithProviders(<OrdersPage />, { initialPath: '/orders' })
    const table = await screen.findByRole('table')

    // The fixtures only carry BDAL/BOPT; the live DB also has BAMS/BMID. The table
    // must still surface the underlying code (e.g. through mock fixtures) as the
    // canonical label so a populated area is never hidden behind a dash.
    const tableText = table.textContent ?? ''
    // ZJ-BLSMS is the label the live DB exposes for `ID_Area = 'BDAL'`.
    expect(tableText).toContain('ZJ-BLSMS')
    // ZI-BOPT is the label for `ID_Area = 'BOPT'`.
    expect(tableText).toContain('ZI-BOPT')
    // At least one of the live product labels (prefix `BDAL `) is present.
    expect(tableText).toMatch(/BDAL (LAB GC|MALDI|MRMS|OTOF|TIMSTOF)/)
  })

  it('renders the Encomenda column as "—" only for genuinely null values', async () => {
    renderWithProviders(<OrdersPage />, { initialPath: '/orders' })
    await screen.findByRole('table')

    // Every "—" cell should be exactly "—" — no whitespace placeholders leaking.
    const cellTexts = Array.from(document.querySelectorAll('td')).map((el) => el.textContent ?? '')
    const dashedCells = cellTexts.filter((text) => text.includes('—'))
    expect(dashedCells.length).toBeGreaterThan(0)
    for (const text of dashedCells) {
  ...
  it('exposes filter controls with accessible names for screen readers', async () => {
    ...
    expect(await screen.findByRole('checkbox', { name: 'BOPT' })).toBeInTheDocument()
