    ...
    mutation.mutate(
      {
        ...
        ID_Tp_Revenue: Number(form.ID_Tp_Revenue),
        ...
        Kit: kit,
        Kit_Amount:
          kit && form.Kit_Amount != null ? Number(form.Kit_Amount) : null,
      },
      { onSuccess: (order) => navigate(`/orders/${order.ID_Order}`) },
    )
  }

  if (user.role === 'viewer') return <div className="p-6 text-sm text-danger">No permission to create orders.</div>
  return (
    <div className="mx-auto w-full max-w-4xl px-6 py-4">
      ...
      <form onSubmit={save} className="rounded-lg border border-border bg-surface p-5 shadow-sm">
        <div className="mb-5 grid gap-4 sm:grid-cols-2">
          <label className="text-xs font-semibold">Order date<DatePicker aria-label="Order date" value={String(form.DT_Order ?? '') || null} onChange={(next) => set('DT_Order', next ?? '')} /></label>
          ...
          <ClientCombobox
            ...
            error={clientError}
          />
          <label className="text-xs font-semibold">Revenue Type<Select required value={String(form.ID_Tp_Revenue ?? '')} onChange={(e) => set('ID_Tp_Revenue', e.target.value)}><option value="">Select</option>{revenueTypes.map((o) => <option key={String(o.id)} value={String(o.id)}>{o.label}</option>)}</Select></label>
          <label className="text-xs font-semibold">Área<Select required value={String(form.ID_Area ?? '')} onChange={(e) => setArea(e.target.value)}><option value="">Select</option>{(areasQuery.data ?? []).map((o) => <option key={String(o.id)} value={String(o.id)}>{o.label}</option>)}</Select></label>
          <label className="text-xs font-semibold">Product type<Select required value={String(form.ID_Tipo ?? '')} onChange={(e) => set('ID_Tipo', e.target.value)}><option value="">Select</option>{tipos.map((o) => <option key={String(o.id)} value={String(o.id)}>{o.label}</option>)}</Select></label>
          <label className="text-xs font-semibold">Produto<Select required value={String(form.ID_Produto ?? '')} onChange={(e) => setProduto(e.target.value)} disabled={!form.ID_Area}><option value="">Select</option>{(produtosQuery.data ?? []).map((o) => <option key={String(o.id)} value={String(o.id)}>{o.label}</option>)}</Select></label>
          <label className="text-xs font-semibold">Instrumento<Select value={String(form.ID_Instrumento ?? '')} onChange={(e) => set('ID_Instrumento', e.target.value)} disabled={!form.ID_Produto}><option value="">Select</option>{(instrumentosQuery.data ?? []).map((o) => <option key={String(o.id)} value={String(o.id)}>{o.label}</option>)}</Select></label>
          ...
          <label className="text-xs font-semibold">Customer PO<Input value={String(form.PO_Cliente ?? '')} onChange={(e) => set('PO_Cliente', e.target.value)} /></label>
          ...
          <label className="text-xs font-semibold sm:col-span-2">Notes<textarea className="mt-1 min-h-32 w-full rounded-md border border-border bg-surface p-3 text-sm" value={String(form.Obs ?? '')} onChange={(e) => set('Obs', e.target.value)} /></label>
        </div>
        ...
        <div className="flex flex-wrap justify-end gap-2">
          <Button type="button" variant="secondary" onClick={() => navigate('/orders')}><ArrowLeft className="size-4" /> Cancel</Button>
          <Button type="submit" disabled={mutation.isPending}><Plus className="size-4" /> {mutation.isPending ? 'Saving...' : 'Create order'}</Button>
        </div>
      </form>
    </div>
  )
...
function ClientCombobox({
  ...
  return (
    <div ref={containerRef} className="relative text-xs font-semibold sm:col-span-2">
      ...
      {error && <p role="alert" className="mt-1 text-xs font-normal text-danger">{error}</p>}
      {open && (
        <ul
          id={listboxId}
          role="listbox"
        ...
        >
          {clients.length === 0 ? (
            <li className="px-3 py-2 text-sm font-normal text-foreground/50">
              {search ? 'Sem resultados.' : 'Escreva para pesquisar.'}
            </li>
          ) : (
            clients.map((client) => (
              <li key={client.ID_Cliente} role="option" aria-selected={false}>
                ...
                >
                  <span className="text-foreground">{client.nome ?? `Client ${client.ID_Cliente}`}</span>
                  {client.no_PHC != null && (
