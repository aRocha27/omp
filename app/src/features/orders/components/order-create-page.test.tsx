...
vi.mock('@/services', async (importActual) => {
  const actual = (await importActual()) as typeof import('@/services')
  return {
    ...actual,
    createRepositories: () => ({
...
describe('OrderCreatePage', () => {
  ...
  it('renders a required Revenue Type select', async () => {
    renderWithProviders(<OrderCreatePage />, { initialPath: '/orders/new', initialRole: 'editor' })
    const revenueSelect = await screen.findByLabelText('Revenue Type')
    expect(revenueSelect).toBeRequired()
  })
  ...
  it('lists clients filtered by the search text in a single combobox', async () => {
    renderWithProviders(<OrderCreatePage />, { initialPath: '/orders/new', initialRole: 'editor' })
    const search = await screen.findByPlaceholderText('Pesquisar cliente...')
  ...
  async function fillRequiredSelects() {
    await selectOption('Área', 'BDAL')
    await selectOption('Product type', 'INSTR')
    await selectOption('Produto', '5')
  ...
  it('cascades Área → Produto and resets the child when the parent changes', async () => {
    ...
    const produtoSelect = await screen.findByLabelText('Produto')
    await waitFor(() => {
