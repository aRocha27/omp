...
import {
  defaultRecognitionReportFilters,
...
export function RecognitionPage() {
  const [filters, setFilters] = useState<RecognitionReportFilters>(() =>
    defaultRecognitionReportFilters(),
  )
  ...
  const facetsQuery = useRecognitionReportFacets(searchFilters)
  ...
  /**
   * The Recognition dimensions mix strings and a single number
   * (`yearRecognition`). The `RecognitionFacetOption.id` carries the
   * canonical value — string for the string dimensions, number for the
   * year. `null` ids are filtered out so the dropdown only shows values the
   * UI can render (a `null` id has no display text and falls back to `—`,
   * which would be a meaningless dropdown entry).
   */
  const yearOptions = (facets?.yearRecognition ?? [])
    .map((entry) => entry.id)
    .filter((value): value is number => typeof value === 'number')
  const areaOptions = (facets?.area ?? [])
    .map((entry) => entry.id)
    .filter((value): value is string => typeof value === 'string')
  const grpReportOptions = (facets?.grpReport ?? [])
    .map((entry) => entry.id)
    .filter((value): value is string => typeof value === 'string')
  const tipoOptions = (facets?.tipo ?? [])
    .map((entry) => entry.id)
    .filter((value): value is string => typeof value === 'string')
  const produtoOptions = (facets?.produto ?? [])
    .map((entry) => entry.id)
    .filter((value): value is string => typeof value === 'string')
  const tpReconhecimentoOptions = (facets?.tpReconhecimento ?? [])
    .map((entry) => entry.id)
    .filter((value): value is string => typeof value === 'string')
  const encomendaOptions = (facets?.encomendaCliPHC ?? [])
    .map((entry) => entry.id)
    .filter((value): value is string => typeof value === 'string')
  ...
  useEffect(() => {
    ...
    // This effect intentionally reconciles selection with the latest result set.
    ...
    <div className="flex h-full flex-col gap-4">
      ...
      <div className="shrink-0 rounded-lg border border-border bg-surface p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-2">
          ...
          />
          <MultiSelectFilter<string>
            label="TP_Reconhecimento"
            options={tpReconhecimentoOptions}
            selected={filters.tpReconhecimento}
            onChange={(next) => patchFilter('tpReconhecimento', next)}
            emptyLabel="No recognition types loaded"
          />
          <MultiSelectFilter<string>
        ...
        <div className="mt-3 flex flex-col gap-1.5">
          ...
          <ActiveFilterChips<string>
            label="TP_Reconhecimento"
            selected={filters.tpReconhecimento}
            onRemove={(value) => patchFilter('tpReconhecimento', filters.tpReconhecimento.filter((v) => v !== value))}
            onClear={() => patchFilter('tpReconhecimento', [])}
          />
          <ActiveFilterChips<string>
            label="Encomenda"
            selected={filters.encomendaCliPHC}
      ...
      <div className="flex-1 min-h-0 overflow-y-auto rounded-lg border border-border bg-surface shadow-sm">
        {(facetsQuery.isPending && !facets) || rowsQuery.isPending ? (
          <div className="p-6">
