...
const snapshot: DashboardSnapshot = {
  ...
  pendingRecognition: [
    {
      idOrder: 2002,
      encomendaCliPHC: 'PEND-001',
      client: 'Client Beta',
      area: 'BOPT',
      type: 'SERVICE',
      product: 'Preventive maintenance',
      sellPrice: 24000,
      totalRecognition: 12000,
      difference: 12000,
    },
  ],
  warrantyMissing: [
  ...
  notFullyInvoicedTotal: 21,
  recentOrders: [recentOrder],
  waitingPoOrders: [{ idOrder: 4001, encomendaCliPHC: null, client: 'Waiting Client' }],
  introduzirSapOrders: [{ idOrder: 4002, encomendaCliPHC: null, client: 'SAP Client' }],
}
...
describe('DashboardPage', () => {
  ...
  it('renders the KPI cards, trend, recognition queue, and recent orders', async () => {
    ...
    expect(screen.getAllByText('Amount to invoice').length).toBe(1)
    ...
    expect(screen.getByRole('button', { name: /Jan\. NOB/i })).toBeInTheDocument()
    ...
    expect(screen.getByText('Warranty start missing')).toBeInTheDocument()
    expect(screen.getByText('W-001')).toBeInTheDocument()
    expect(screen.getAllByLabelText('Showing 1 out of 21')).toHaveLength(2)

    expect(screen.getAllByText('Amount to invoice').length).toBeGreaterThan(0)
    expect(screen.getByText('Total to invoice')).toBeInTheDocument()
    expect(screen.queryByText('Total value')).not.toBeInTheDocument()
    expect(screen.getByText('INV-001')).toBeInTheDocument()
    expect(screen.getByText('€10,000.00')).toBeInTheDocument()
    expect(screen.getByText('Pending recognition')).toBeInTheDocument()
