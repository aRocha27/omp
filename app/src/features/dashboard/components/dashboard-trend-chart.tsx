...
export function DashboardTrendChart({ points }: DashboardTrendChartProps) {
  ...
  const chartYearLabel = useMemo(() => {
    ...
    const firstYear = yearLabel(points[0].monthStart)
    const lastYear = yearLabel(points[points.length - 1].monthStart)
  ...
  const axisTicks = useMemo(
    () => [graphCeiling, graphCeiling * 0.75, graphCeiling * 0.5, graphCeiling * 0.25, 0],
    [graphCeiling],
  )
        ...
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-foreground/60">
              Revenue and NOB for the last {points.length || 7} months.
            </p>
          </div>
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="text-sm font-semibold text-foreground">Monthly operational trend</h3>
          <p className="mt-1 text-xs text-foreground/60">
             {chartYearLabel} NOB vs Revenue. Rolling {points.length}-month window. Chart scale is
            capped at {formatPrice(graphCeiling)}.
          </p>
        </div>
            ...
            {activePoint ? (
              <>
                <div className="font-medium text-foreground">
                  {monthLabel(activePoint.monthStart)}
                </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-border/70 bg-background/70 p-5">
        <div className="min-w-[720px]">
          <div className="flex gap-4">
            ...
            >
              <div
                aria-hidden
                className="pointer-events-none absolute inset-0 flex flex-col justify-between pb-8"
              >
                {axisTicks.map((tick) => (
                  ...
                  return (
                    <button
                      ...
                      className="group flex min-w-0 flex-1 flex-col items-center gap-3 rounded-md px-1 pb-1 pt-2 outline-none focus-visible:ring-2 focus-visible:ring-primary"
                       aria-label={`${monthLabel(point.monthStart)}. NOB ${formatPrice(point.nob)}. Revenue ${formatPrice(point.revenue)}.`}
                    ...
                    >
                      ...
                      <span
                        className={`text-[11px] font-medium ${active ? 'text-foreground' : 'text-foreground/60'}`}
                      >
