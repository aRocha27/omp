...
export function ClientsPage() {
  ...
  const totalQuery = useClients(emptyFilters)
  ...
  return (
    <div>
      ...
      <section
        ...
        className="mb-4 flex w-fit min-w-44 items-center justify-between gap-6 rounded-lg border border-border bg-surface px-4 py-3 shadow-sm"
      >
        <span className="text-xs font-medium text-foreground/60">Total clients</span>
        <strong className="text-2xl font-semibold tabular-nums text-foreground">
          {totalQuery.isPending ? '—' : (totalQuery.data?.length ?? 0)}
        </strong>
      </section>
