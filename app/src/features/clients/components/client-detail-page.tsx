...
import { Pencil, Save, X, CircleAlert } from 'lucide-react'
...
function BackButton() {
  return <UiBackButton to="/clients" />
}

function notFoundState(description: string) {
  return <NotFoundState description={description} title="Client not found" backTo="/clients" />
}
...
function clientToDraft(client: Client): ClientFormDraft {
  return {
    ...
    fax: client.fax ?? '',
    zona: client.zona ?? '',
    Defense: client.Defense == null ? 'false' : String(client.Defense),
  }
}
...
function draftToPatch(draft: ClientFormDraft, client: Client): ClientUpdatePatch {
  ...
  setIfChanged('Defense', draft.Defense === 'true')
    ...
    <>
      ...
      <section className="mb-6">
        ...
        <FieldGrid>
          ...
          <ReadField label="Defense">{client.Defense ? 'Yes' : 'No'}</ReadField>
        </FieldGrid>
      </section>
    </>
  )
