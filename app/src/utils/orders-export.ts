...
import {
  resolveAreaExport,
  resolveInstrumentoExport,
  resolveOrderTypeExport,
  resolveProdutoExport,
  resolveRevenueExport,
  resolveTipoExport,
  resolveWarrantyExport,
...
function cellFor(order: OrderSummary, key: keyof OrderSummary): string | number | null {
  ...
  switch (key) {
    case 'ID_Tp_Order':
      return resolveOrderTypeExport(order)
    case 'ID_Area':
      return resolveAreaExport(order)
    case 'ID_Tipo':
      return resolveTipoExport(order)
    case 'ID_Produto':
      return resolveProdutoExport(order)
    case 'ID_Instrumento':
      return resolveInstrumentoExport(order)
    case 'ID_Tp_Warranty':
      return resolveWarrantyExport(order)
    case 'ID_Tp_Revenue':
      return resolveRevenueExport(order)
    case 'DT_Order':
 ...
 *
 * Pattern: `order-<active-filter-values>-<dd-mm-yyyy>.xlsx`. Only active
 * filters contribute a slug — empty/cleared filters are skipped, so a
 * freshly opened list exports as `order-26-08-2026.xlsx`. Slugs are just
 * the filter values (slugified, no label prefix like `type-`), joined by
...
export function buildOrdersFileName(
  ...
  const slugs: string[] = []
  if (filters.clientName) slugs.push(slugify(filters.clientName)!)
  if (filters.orderType) slugs.push(slugify(filters.orderType)!)
  if (filters.area) slugs.push(slugify(filters.area)!)
  if (filters.tipo) slugs.push(slugify(filters.tipo)!)
  if (filters.product) slugs.push(slugify(filters.product)!)
  if (filters.instrument) slugs.push(slugify(filters.instrument)!)
  if (filters.sapOrder) slugs.push(slugify(filters.sapOrder)!)
  if (filters.dateFrom) slugs.push(slugify(filters.dateFrom)!)
  if (filters.dateTo) slugs.push(slugify(filters.dateTo)!)
