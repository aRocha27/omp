...
describe('DatePicker', () => {
  ...
  it('opens the calendar instead of the numeric keyboard on touch-sized field taps', () => {
    const showPicker = vi.fn()
    ...
    try {
      render(<DatePicker aria-label="Order date" value={null} onChange={() => {}} />)
      fireEvent.pointerDown(screen.getByLabelText('Order date'), { pointerType: 'touch' })
      expect(showPicker).toHaveBeenCalledTimes(1)
    } finally {
      Object.defineProperty(window, 'matchMedia', {
        configurable: true,
