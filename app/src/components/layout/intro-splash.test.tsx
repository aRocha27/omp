...
function getVideo(): HTMLVideoElement {
  const dialog = screen.getByRole('dialog', { name: /bruker intro/i })
...
describe('IntroSplash', () => {
  ...
  it('points at the bundled BRUKER startup video by default', () => {
    render(<IntroSplash />)
    expect(getVideo().getAttribute('src')).toMatch(/BRUKER_logo_startup_animation/)
  })

  it('dismisses itself when the Skip button is clicked', () => {
    render(<IntroSplash />)
    expect(screen.getByRole('dialog', { name: /bruker intro/i })).toBeInTheDocument()

    fireEvent.click(screen.getByRole('button', { name: /skip intro/i }))

    expect(screen.queryByRole('dialog', { name: /bruker intro/i })).not.toBeInTheDocument()
  })
