...
The OrdersPaiSoft application is in its frontend/UI-first phase. The user now has the real production database and wants the Administration page to actually connect to SQL Server, list its tables, and sync the orders table — not a simulation.
...
- **Administration API authentication:** every deployment requires `ADMIN_API_TOKEN`; the UI sends it only in the `Authorization: Bearer` header and never persists it. There is no source-address-based authentication exception, so a local or reverse proxy cannot accidentally bypass the token.
