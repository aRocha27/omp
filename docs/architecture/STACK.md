...
This file defines the default frontend stack for the current UI-first phase.
...
There is no requirement to build the production backend during the current UI-first phase.

The future backend must connect to the existing database.
...
but this is not a current frontend dependency.

Do not couple the frontend implementation to a backend framework.
